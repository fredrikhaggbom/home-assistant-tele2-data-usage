...
from .const import (DOMAIN, POLL_INTERVAL, RES_UNLIMITED, RES_LIMIT, RES_USAGE, RES_DATA_LEFT, RES_PERIOD_START, RES_PERIOD_END, Tele2ApiResult)
...
class Tele2Session:
    ...
    def __init__(self, hass: HomeAssistant, config: Config):
        ...
        if CONF_NAME in config and POLL_INTERVAL in config and CONF_USERNAME in config and CONF_PASSWORD in config:
            _LOGGER.debug("Initing Tele2Session with: %s, %s, %s, %s", name, str(pollInterval), username, "****")
        ...
        if (resp.status_code == 401 or resp.status_code == 403) and self.tries < 1:
            ...
            if Tele2ApiResult.buckets in data and len(data["buckets"]) > 0:
                ...
                if Tele2ApiResult.startDate in bucket:
                    startDate = datetime.datetime.strptime(bucket[Tele2ApiResult.startDate], '%Y-%m-%d').date()
                    self._data[RES_PERIOD_START] = startDate
                if Tele2ApiResult.endDate in bucket:
                    endDate = datetime.datetime.strptime(bucket[Tele2ApiResult.endDate], '%Y-%m-%d').date()
